...
bool signupOK = false;                 
float freq_firebase,freq_mask;
int vol_firebase=0,vol_mask=0;
...
STATE_FIREBASE State_Firebase = STATE_FIREBASE1;
/*---------------Button Define-------------------------------*/
int volume_fm = 0b111;    //volume value.
...
void loop() {
  Read_button();
  set_announce_time();
  read_freq = getFrequency();
  if(Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 250 || sendDataPrevMillis == 0)){
    sendDataPrevMillis = millis();
    switch(State_Firebase){
      case STATE_FIREBASE1:
        Firebase.RTDB.setFloat(&fbdo, "FM_IoT/Frequency",read_freq);      
        State_Firebase = STATE_FIREBASE2; break;
      case STATE_FIREBASE2:  
        Firebase.RTDB.setFloat(&fbdo, "FM_IoT/Volume",volume_fm);         
        State_Firebase = STATE_FIREBASE3; break;
      case STATE_FIREBASE3:  
        Serial.printf("%s\n", Firebase.RTDB.getInt(&fbdo, F("/FM_IoT/Volume_control")) ? String(vol_firebase=fbdo.to<int>()).c_str() : fbdo.errorReason().c_str());
        if(vol_mask!=vol_firebase){
          vol_mask = vol_firebase;
          volume_fm = vol_firebase;
          setVolume(volume_fm);
        }
        State_Firebase = STATE_FIREBASE4; break;
      case STATE_FIREBASE4:
        Serial.printf("%s\n", Firebase.RTDB.getFloat(&fbdo, F("/FM_IoT/Frequency_control")) ? String(freq_firebase=fbdo.to<float>()).c_str() : fbdo.errorReason().c_str());
        if(freq_mask!=freq_firebase){
          freq_mask = freq_firebase;
          tanso_fm = freq_firebase;
          setFrequency(tanso_fm);
          }  
        State_Firebase = STATE_FIREBASE1; break;
    }      
  }
  oled_display();
}
/*   ---------------------Dưới đây là các hàm khởi tạo-----------------------------*/
void firebase_wifi_init(){
...
void setVolume( unsigned int volume) { // ham thay doi am luong.
...
void Process_Noise(){//ham xu ly tieng on
  off_volume();
